...
-- 2. Setup the reference/initial board
...
-- FIX: Locking both tables to allow the INSERT INTO ... SELECT
LOCK TABLES `board_empty` WRITE, `board` READ;
INSERT INTO `board_empty` SELECT * FROM `board`;
UNLOCK TABLES;

-- 3. Setup game status (Fixed 'aborted' typo)
...
BEGIN
  ...
  -- Determine the next stack height at destination
  SELECT IFNULL(MAX(stack_order) + 1, 0) INTO target_stack_order 
  FROM board 
  WHERE pos = p_to;

  -- Perform move if a piece exists
